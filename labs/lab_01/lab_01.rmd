...
**A1** Computer, Math, and Natural Sciences had the highest average of seats, with 440. CMNS was the most surprising to me, because an average class size of 440 seems absolutely crazy to me (where do they meet? Is it all big lectures??).
...
**A2** The greatest number of times an individual class has been offered is 29 times.
...
**A3** Keith Yearwood taught the most classes.
...
**A4** It would be much easier to see the answer if we could have cleaned the data to remove all the NAs. That said, I'm quite confident that we got the right answer, because that was the first non-NA that we found.
...
**A5** I asked the question, "Who are the professors with the highest number of syllabuses uploaded?" The answer was George Syrmos, with 473 total syllabuses uploaded across 22 classes. This is very interesting, because it seems like way too many for one person.
...
umd_courses |> 
  ...
  summarise(
    count_classes = n(),
    total_syllabus = sum(syllabus_count, na.rm=TRUE)
  ) |> 
...
# which teacher has taught the highest number of classes, for fun :)
umd_courses |> 
  group_by(instructors) |> 
  summarise(
    count_classes = n()
