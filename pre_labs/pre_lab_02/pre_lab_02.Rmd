...
**Answer:** Making a new variable ensures that we are creating a new table, instead of chopping up the original one.
...
**Answer:** The expenditure_date column is now dates
...
**Answer:** My guess is that lubridate's floor date is the first day of the month, and that represents the month and allows all the other stuff to happen.
...
**Answer:** It's adding all the vote totals in each county (and putting it into a new column), then dividing Moore's total by the total, which gets you the percentage expressed as a decimal. Mutate then puts this calculation into another new column.
...
**Answer:** By adding the final line of code, we have now sorted all of the counties from greatest percentage to least percentage of people who voted for Moore.
...
**Answer:** It's probably interesting to his campaign people, but I don't know enough about Maryland to know if there's any interesting reasons why Garrett county would have that low of a percentage. Still, it's probably more interesting to think about a potential reason for this instead of why Prince George's county had the highest percentage.
...
**Answer:** str_to_lower converts to lower case. str_to_title converts to title case, where the first letter of the word is capitalized.
...
**Answer:** It's going through the whole address column and looking for Maryland, and adding any matches it finds to the new column with the new abbreviation. Then it's doing the same for California. Then Washington, and so on until it reaches the end, and then any rows that didn't match any of the listed states will have their corresponding row in the new column populated with NA.
...
**Answer:** Not really. Why is there so much money going to places that don't have addresses. (If this question is in regard to the process of writing the code to do this and I just misunderstood, it does makes sense to me)
...
1.  Are we sure that the states listed in the earlier code are the only ones in this data set? How do we search for more?

2.  How much did Maryland spending change from 2021 to 2022?

3.  Who received the most money in each year?
