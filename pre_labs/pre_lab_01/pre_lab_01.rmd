...
In the space below, write the name of the course from the first row of the "title" column. **Answer:** Directed Research
...
**Answer:** Calls up the file, identifies and sorts the column we want to look at, gets a count of each version of the term column, saves that count to a new variable so that it can be a column in a new table
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer:** The rows are now sorted from greatest to least
...
**Answer:** It now lists the number of each classes per term, per department. This was achieved by adding department to the group_by statement
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer:** Added a total_seats variable which adds all the seats in each class together, per term, and outputs that in the table
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer:** Added the mean and median functions to the code, which added them to the table. All of the top terms in this new table have roughly the same average AND median number of seats
...
**Answer:** A lot has changed. We are now grouping by department instead of term, and counting both the number of classes and the total number of seats in each department across all terms. For each department, we have the mean and median number of seats in each class, along with the maximum and minimum number of seats in a class.
...
1.  Who are the professors with the highest number of syllabuses uploaded?

2.  Which professors teach the classes with the most seats?

3.  Which individual classes in each term have had the largest/fewest number of seats?
